...
echo "Saving customer session video..."
VSTOP1=$($PW video-stop 2>&1 || true)
VPATH1=$(echo "${VSTOP1}" | grep -oP '(?<=\[Video\]\()[^)]+\.webm(?=\))' | sed 's|\\|/|g' || echo "")
if [ -n "${VPATH1}" ] && [ -f "${VPATH1}" ]; then
  cp "${VPATH1}" "${REPORT_DIR}/video_customer.webm" 2>/dev/null || true
elif [ -f "${REPORT_DIR}/video_customer.webm" ]; then
  echo "  video_customer.webm already in place"
fi
$PW close
...
echo "Saving agent session video..."
VSTOP2=$($PW video-stop 2>&1 || true)
VPATH2=$(echo "${VSTOP2}" | grep -oP '(?<=\[Video\]\()[^)]+\.webm(?=\))' | sed 's|\\|/|g' || echo "")
if [ -n "${VPATH2}" ] && [ -f "${VPATH2}" ]; then
  cp "${VPATH2}" "${REPORT_DIR}/video_agent.webm" 2>/dev/null || true
elif [ -f "${REPORT_DIR}/video_agent.webm" ]; then
  echo "  video_agent.webm already in place"
fi
$PW close
echo "Agent done."
...
<div class="container">
  ...
  <h2>操作動画</h2>
  <p style="font-size:13px;color:#374151;">以下の動画ファイルでテスト実行中のブラウザ操作を確認できます（WebM形式、Chrome/Edge/Firefoxで再生可能）。</p>
  <div style="display:flex;gap:16px;flex-wrap:wrap;">
    <div style="flex:1;min-width:280px;">
      <p style="font-size:12px;color:#6b7280;margin-bottom:6px;">Customer セッション（New Ticket 作成）</p>
      <div style="background:#1e293b;color:#e2e8f0;padding:10px 14px;border-radius:6px;font-family:monospace;font-size:12px;">
        ${REPORT_DIR}/video_customer.webm
      </div>
    </div>
    <div style="flex:1;min-width:280px;">
      <p style="font-size:12px;color:#6b7280;margin-bottom:6px;">Agent セッション（Submit / Assign / Escalate）</p>
      <div style="background:#1e293b;color:#e2e8f0;padding:10px 14px;border-radius:6px;font-family:monospace;font-size:12px;">
        ${REPORT_DIR}/video_agent.webm
      </div>
    </div>
  </div>
